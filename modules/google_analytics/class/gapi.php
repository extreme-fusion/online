...
class Gapi
{
  ...
  public function requestAccountData ($start_index = 1, $max_results = 20)
  {
    ...
    {
	  $this->_error = array(__('GAPI: Failed to request account data. Error: :error.', array(':error' => strip_tags($response['body']))));
	}
  }
  ...
  public function requestReportData ($report_id, $dimensions, $metrics, $sort_metric = NULL, $filter = NULL, $start_date = NULL, $end_date = NULL, $start_index = 1, $max_results = 30)
  {
    ...
    {
      $this->_error=array(__('GAPI: Failed to request report data. Error: :error.', array(':error' => ! empty($response['body']) ? strip_tags($response['body']) : __('There is no connection with Google Analytics server.'))));
    }
  }
  ...
  public function __call ($name, $parameters)
  {
    ...
    {
      $this->_error = array(__('No such function: :name.', array(':name' => $name)));
    }
    ...
    $this->_error = array(__('No valid root parameter or aggregate metric called: :name.', array(':name' => $name)));
  }
}
