...
<div class="info">{if $active}{i18n('Logs are enabled.')}{else}{i18n('Logs are disabled.')}{/if}</div>
	<div>	
		<span class="box-left">
			<a href="{$ADDR_ADMIN_PAGES}settings_logs.php" class="tip" title="{i18n('Ustawienia')}">{i18n('Ustawienia')}</a>
		</span>
		{if $logs}
...
{if $logs}
	...
	<div class="clear"></div>
	<div class="tbl2">
		<div class="grid_2 bold center">{i18n('Date:')}</div>
	...
	{section=logs}
		<div class="tbl {$logs.row_color}">
			<div class="grid_2 center">{$logs.date}</div>
